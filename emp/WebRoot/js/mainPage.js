	...
	$.ajax({
		...
		success:function(data){
			...
			for(var i=0;i<6&&i<data.rows.length;i++)
			{
					...
					+'<a onclick="toMenuDetail('+data.rows[i].aac101+')" style="cursor:pointer;"><img src="'+data.rows[i].aac108+'" style="widtn:180px;height:100px;"></a>'
					+'<p><a onclick="kitchen('+data.rows[i].aab101+')" style="cursor:pointer;">'+data.rows[i].aab102+'</a>创建&nbsp;&nbsp;'+data.rows[i].ac02count+'做过</p>'
					+'<a onclick="toMenuDetail('+data.rows[i].aac101+')" style="cursor:pointer;">更多细节...</a>'
					+'</br>'
					+'</div>'
					+'</div>'
					+'</div>';
		...
		success:function(data){
			...
			{	
				html+='<div class="layui-card">'
					 ...
					 +'<a onclick="kitchen('+data.rows[i].aab101+')" style="cursor:pointer;"><img class="layui-nav-img" src="'+data.rows[i].aab106+'" style="width:100px;height: 80px;border:1px"></a>'
					 +'</div>'
					 +'<div class="layui-col-md5">'
					 +'<a onclick="kitchen('+data.rows[i].aab101+')" style="cursor:pointer;">'+data.rows[i].aab102+'</a><span>'+data.rows[i].aab108+'关注</span>'
					 +'<p><span>'+data.rows[i].ac01count+'食谱&nbsp;</span><span>'+data.rows[i].ac02count+'个作品</span></p>'
					 +'<a  onclick="kitchen('+data.rows[i].aab101+')" style="cursor:pointer;">'
					 +'更多细节...'
					 +'</a>'
					 +'</div>'
					 +'</div>'
			...
			{	
				html+='<div class="layui-row layui-col-space30">'
					...
					/* +'<br/>'
					 ...
					 +'<a onclick="ActiDetail('+data.rows[i].aae101+');" style="float:right;color:#c2c2c2;cursor:pointer;">更多细节...</a>'
					 +'</div>'
					 +'</div>'
					 +'</div>';
			}
